...
\label{sec:Critica}
Il modello presentato nella \autoref{subsec:DescModello} e utilizzato per gli esempi nella \autoref{sec:Applicazioni} non descrive adeguatamente un particolare esempio di interesse.
...
Pensiamo a $\widehat{\gamma}_1=(r_1, r_2, r_3)$ come ad una superstrada. In situazioni di traffico moderato ($\widehat{\zeta}$ e $\widecheck{\zeta}$ non troppo elevati) entrambe le popolazioni la utilizzano.
...
per cui queste ultime due strade sono opzioni allettanti per i guidatori è la precedenza che hanno in uscita dalle scorciatoie: i veicoli provenienti da $r_4$ hanno precedenza sui veicoli
...
Ciò è dato dal fatto che i guidatori \textit{check} sono costretti a cedere la precedenza, aumentando il tempo di viaggio già elevato a causa della forte presenza di veicoli nella rete. Facendo crescere il costo delle
...
\end{tabular}
\end{figure}
Il modello definito in \autoref{sec:Modello} non tiene conto delle precedenze: i guidatori \textit{hat} smettono progressivamente di utilizzare le scorciatoie
(dato che i costi delle stesse aumentano), ma il tempo di percorrenza della rete per i veicoli \textit{check} non migliora. 

Se vogliamo descrivere meglio il comportamento reale è necessario che il costo di una strada dipenda anche dalla popolazione presente su un'altra.
...
validità. Modificare leggermente l'enunciato e la dimostrazione è comunque possibile. È bene notare in ogni
caso che in questa sezione ci stiamo limitando ad analizzare una singola istanza di un nuovo modello, la quale dà risultati coerenti e significativi.
...
\bigl(
\widehat{\tau}_1(\widehat{\eta},\widecheck{\eta}),
\dots,
\widehat{\tau}_N(\widehat{\eta},\widecheck{\eta})
\bigr)
\\[1em]
\widecheck{\tau}: & [0,1]^N\times[0,1]^N & \to & \mathbb{R}_+^N \cup \{+\infty\}\\
& (\widehat{\eta},\widecheck{\eta}) & \mapsto &
\bigl(
\widecheck{\tau}_1(\widehat{\eta},\widecheck{\eta}),
\dots,
\widecheck{\tau}_N(\widehat{\eta},\widecheck{\eta})
\bigr)
\end{array}
\end{split}
\end{equation}

Per quanto riguarda la monotonia, possiamo introdurre la relazione d'ordine parziale:
\begin{equation}
\label{eq:16}
\eta', \eta'' \in [0,1]^N, \quad \eta'\leq\eta''\rightleftharpoons\forall i \in {1,...,N}\quad \eta'_i\leq\eta''_i
\end{equation}
Questo ci consente di riscrivere l'\autoref{eq:15}:
\begin{equation}
\label{eq:17}
\begin{split}
    \forall \widehat{\eta}, \widecheck{\eta}', \widecheck{\eta}''\in[0,1]^N\quad\widecheck{\eta}'\leq\widecheck{\eta}''\implies\widehat{\tau}(\widehat{\eta}, \widecheck{\eta}')\leq\widehat{\tau}(\widehat{\eta}, \widecheck{\eta}''),\,\widecheck{\tau}(\widehat{\eta}, \widecheck{\eta}')\leq\widecheck{\tau}(\widehat{\eta}, \widecheck{\eta}'')\\
    \forall \widehat{\eta}', \widehat{\eta}'', \widecheck{\eta}\in[0,1]^N\quad\widehat{\eta}'\leq\widehat{\eta}''\implies\widehat{\tau}(\widehat{\eta}', \widecheck{\eta})\leq\widehat{\tau}(\widehat{\eta}'', \widecheck{\eta}),\,\widecheck{\tau}(\widehat{\eta}', \widecheck{\eta})\leq\widecheck{\tau}(\widehat{\eta}'', \widecheck{\eta})
\end{split}
\end{equation}
...
Variamo il numero di veicoli da $\widehat{\zeta} = \widecheck{\zeta} = 1$ a $\widehat{\zeta} = \widecheck{\zeta} = 101$ (vedi \autoref{script:24}) e
confrontiamo i grafici ottenuti con i risultati ottenuti senza le precedenze:
...
\end{tabular}
\caption*{Tempo di viaggio \textit{hat} al variare delle popolazioni, a sinistra senza precedenze, a destra con le precedenze}
\end{figure}
...
\end{tabular}
\caption*{Tempo di viaggio \textit{check} al variare delle popolazioni, a sinistra senza precedenze, a destra con le precedenze}
\end{figure}
...
\end{tabular}
\caption*{Equilibrio di Nash \textit{hat} al variare delle popolazioni, a sinistra senza precedenze, a destra con le precedenze. Viene tralasciato l'Equilibrio di Nash di \textit{check} in quanto sempre costante}
\end{figure}
...
\end{tabular}
\caption*{Tempo di viaggio \textit{hat} al variare del costo delle scorciatoie, a sinistra senza precedenze, a destra con le precedenze}
\end{figure}
...
\end{tabular}
\caption*{Tempo di viaggio \textit{check} al variare del costo delle scorciatoie, a sinistra senza precedenze, a destra con le precedenze}
\end{figure}
...
\end{tabular}
\caption*{A sinistra la variazione dell'Equilibrio di Nash calcolato senza le precedenze. A destra la variazione dell'Equilibrio di Nash con le precedenze.}
\end{figure}
