...
Identifichiamo come percorso principale è $(r_1, r_2, r_3)$. In situazioni di traffico moderato ($\widehat{\zeta}$ e $\widecheck{\zeta}$ non troppo elevati) le due popolazioni lo utilizzano esclusivamente per arrivare a destinazione.
...
sul percorso principale per evitare il traffico intenso. Tentiamo di desccrivere questa situazione con il modello \autoref{sec:Modello}.
...
Se vogliamo descrivere meglio il comportamento reale dobbiamo permettere al costo di una strada di variare rispetto alla popolazione presente su un'altra.
Servirebbe dunque far dipendere i $\tau$ che costituiscono il percorso principale non solo dalla concentrazione di veicoli sulla loro relativa strada,
ma anche da $\theta_4$ e $\theta_5$. Questa necessità comporta una generalizzazione del modello, che non intaccherà altre definizioni e non invaliderà il \autoref{th:1}.
Estendiamo dunque il modello presentato in precedenza:
...
\includegraphics[height=6cm]{../figures/Es4/grafico_check/check_theta_1_Variazione_del_numero_di_individui_nella_popolazione.png} \\
