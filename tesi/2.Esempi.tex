...
\begin{itemize}
    ...
    \item Costruzione dei grafici del tempo di viaggio e dell'equilibrio di Nash dipendenti dalla variazione dei costi delle strade
    \item Costruzione dei grafici del tempo di viaggio e dell'equilibrio di Nash dipendenti dalla variazione delle popolazioni
\end{itemize}
...
\quad
\]
In prima analisi prendiamo l'esempio principe riguardante il Paradosso di Braess \cite{BraessParadox}, presente in svariati trattati sul tema (si veda, per esempio, \cite{Easley_Kleinberg_2010}).
...
\widehat{\tau}_5(\eta) = \delta &
\widecheck{\tau}_5(\eta) = \delta
$$
\end{array}
\end{equation*}
con:
...
Il comportamento paradossale deriva dal fatto che la presenza della strada $r_5$, nonostante abbia costo nullo, peggiora i tempi di viaggio di entrambe le popolazioni.
Infatti se variamo il costo della strada $r_5$, passando da $\delta = 0$ a 
$\delta = 50$ (vedi \autoref{script:18}): 
...
del tempo di viaggio. Notiamo, dunque, che aumentare oltre una certa soglia il costo di una strada è equivalente a rimuoverla dalla rete.

La rimozione di $r_5$ migliora la qualità del viaggio sulla rete. 
...
Se esaminiamo il caso in cui $\widehat{\tau_5}(\eta) = \widecheck{\tau_5}(\eta) = 0$ e facciamo variare 
...
costante nessuno utilizza $r_5$ e si perde il comportamento paradossale della rete: in questo caso anche se $r_5$ ha costo 0 non conviene
percorrerla.

\subsection{Effetti a distanza}
\label{subsec:Es2}

Introduciamo ora una rete particolare, in cui l'aggiunta di una strada utilizzata solo da una popolazione peggiora unicamente il tempo di viaggio dell'altra:
\[
\quad
\begin{tikzpicture}[scale=1.2]
...
Variando il costo della strada $r_6$ \textbf{solamente per la popolazione \textit{hat}}, passando da $\delta=0$ a $\delta=0.5$ otteniamo (vedi \autoref{script:20}):
\newpage
\vspace*{-4cm}
...
Come nella \autoref{subsec:Es1} aumentando il costo di $r_6$ fino a renderla inutilizzabile il tempo di viaggio (per \textit{check}) migliora: osserviamo un altro Paradosso di Braess \cite{BraessParadox}. Questa volta, però, solo una delle due popolazioni
(\textit{check}) risente della variazione di costo di $r_6$, nonostante nessun veicolo \textit{check} la attraversi.
L'aggiunta di $r_6$, che
