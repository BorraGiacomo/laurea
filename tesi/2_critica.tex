...
\label{eq:13}
\end{equation}
Facciamo variare il numero di veicoli nelle due popolazioni da $\widehat{\zeta} = \widecheck{\zeta} = 1$ a $\widehat{\zeta} = \widecheck{\zeta} = 101$. Otteniamo dunque (vedi \autoref{script:22}):
...
\end{tabular}
\end{figure}
Il risultato ottenuto non è esattamente quello atteso: i guidatori \textit{hat} smettono progressivamente di utilizzare le scorciatoie
...
Generalizziamo comunque questa equazione, uscendo dal modello presentato in precedenza, e tentiamo di ottenere un risultato più realistico:
