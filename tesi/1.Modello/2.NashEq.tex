...
Di seguito sono riportate alcune definizioni utili da \cite{MR4911797}. Si noti che l'analisi effettuata in questa sezione non cambia se si utilizzano le funzioni di costo come nell'\autoref{eq:10}.
...
\begin{definition}\label{def:2}
    ...
    se tutti i tempi di viaggio rilevanti, come dalla \autoref{def:1}, di ogni popolazione coincidono, ovvero se:
    \begin{equation*}
        \begin{split}
...
arrivare dall'origine alla destinazione.

Grazie a queste definizioni ed all'\autoref{eq:3}, introduciamo:
...
È detto \textit{tempo di viaggio} all'equilibrio, distintamente per le due popolazioni, il valore assunto dal $T_M$ che caratterizza l'Equilibrio di Nash trovato.

Notiamo che, in uno stato di equilibrio $\theta^*$, $\widehat{T}_M(\theta^*)=\widehat{T}_i(\theta^*)$ $\forall i\in\{0,...,\widehat{n}\}$ e $\widehat{\theta}^*_i$(analogo per la popolazione \textit{check}),
...
Chiamiamo $\mathds{1}_{\widehat{n}}$ il vettore colonna di dimensione $\widehat{n}$ i cui elementi sono tutti 1 (analogo per $\mathds{1}_{\widecheck{n}}$).
...
denominato in seguito \textit{Equilibrio di Nash}.

In \cite{MR4911797} viene dunque verificato che \autoref{eq:6}, per il Teorema di Brouwer, ammette almeno un punto fisso:
\begin{equation}
    \exists\theta^*\in S^{\widehat{n}}\times S^{\widecheck{n}}:\text{ } \mathcal{F}(\theta^*)=\theta^*
    \label{eq:9}
\end{equation}
Ogni punto fisso di $\mathcal{F}$ è un Equilibrio di Nash per la rete stradale, al senso della \autoref{def:3}.
