...
la popolazione \textit{hat} e la popolazione \textit{check}. Se chiamiamo \textit{nodo} l'intersezione di due archi, possiamo identificare un'origine e una destinazione per entrambe le popolazioni,
$(\widehat{\mathcal{O}}, \widehat{\mathcal{D}})$ e $(\widecheck{\mathcal{O}}, \widecheck{\mathcal{D}})$, non per forza distinte.
...
Queste due sottoreti devono essere grafi orientati aciclici (DAG, \textit{Directed Acyclic Graph}) connessi. Si noti che non è necessario imporre queste condizioni
...
rispettivamente i costi della strada $r_h$, $h\in\{1,...,N\}$, per la popolazione \textit{hat} e per la popolazione \textit{check}. In questa trattazione, similmente a \cite{MR4911797}, considereremo i costi come tempi di viaggio,
...
\begin{equation}
    \begin{split}
    \begin{array}{l c c c} //TOGLI, LASCIA REF E TIENI SOLO QUELLA DOPO
        \widehat{\tau}: & [0,1]^N\times[0,1]^N & \to & \mathbb{R}_+^N \cup \{+\infty\}\\
        & \widehat{n}, \widecheck{n} & \mapsto & \widehat{\tau}(\widehat{n}, \widecheck{n}) \quad dove \quad (\widehat{\tau}(\widehat{n}, \widecheck{n}))_h = \widehat{\tau}_h(\widehat{n}_h, \widecheck{n}_h) \quad per \quad h\in\{1,...,N\}\\
        \widecheck{\tau}: & [0,1]^N\times[0,1]^N & \to & \mathbb{R}_+^N \cup \{+\infty\}\\
        & \widehat{n}, \widecheck{n} & \mapsto & \widecheck{\tau}(\widehat{n}, \widecheck{n}) \quad dove \quad (\widecheck{\tau}(\widehat{n}, \widecheck{n}))_h = \widecheck{\tau}_h(\widehat{n}_h, \widecheck{n}_h) \quad per \quad h\in\{1,...,N\}
    \end{array}
    \end{split}
    \label{eq:11}
\end{equation}
