...
Per completezza, possiamo definire $\widehat{\zeta}$ e $\widecheck{\zeta}$, il numero totale di veicoli rispettivamente della popolazione \textit{hat} e \textit{check},
presenti sulla rete.
...
A priori i costi delle due popolazioni sono completamente scollegati. I guidatori \textit{hat} e \textit{check} prendono parte a giochi differenti ma interdipendenti: i $\tau$
possono rappresentare criteri distinti per le due popolazioni. Per esempio la popolazione \textit{hat} attraversa la rete cercando di arrivare a destinazione nel minor tempo possibile, mentre \textit{check}
cerca di minimizzare l'utilizzo di carburante durante il viaggio.

Da un punto di vista modellistico è naturale assumere che:
\begin{equation} 
    \label{eq:15}
    \begin{split}
    \forall \widehat{\eta}, \widecheck{\eta}', \widecheck{\eta}''\in[0,1]\quad\widecheck{\eta}'\leq\widecheck{\eta}''\implies\widehat{\tau}(\widehat{\eta}, \widecheck{\eta}')\leq\widehat{\tau}(\widehat{\eta}, \widecheck{\eta}''),\,\widecheck{\tau}(\widehat{\eta}, \widecheck{\eta}')\leq\widecheck{\tau}(\widehat{\eta}, \widecheck{\eta}'')\\
    \forall \widehat{\eta}', \widehat{\eta}'', \widecheck{\eta}\in[0,1]\quad\widehat{\eta}'\leq\widehat{\eta}''\implies\widehat{\tau}(\widehat{\eta}', \widecheck{\eta})\leq\widehat{\tau}(\widehat{\eta}'', \widecheck{\eta}),\,\widecheck{\tau}(\widehat{\eta}', \widecheck{\eta})\leq\widecheck{\tau}(\widehat{\eta}'', \widecheck{\eta})
    \end{split}
\end{equation}
dato che, al crescere della popolazione presente su una strada, il tempo di viaggio su di essa aumenta.
